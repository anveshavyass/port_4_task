...
def _build_messages(ticket_text: str, repair_context: str = None, invalid_response: Any = None) -> list[dict[str, str]]:
    instructions = (
        ...
        "urgency for one over the other), that issue is primary, full stop, no matter which order the issues "
        "were mentioned in; (2) otherwise, if one of the issues involves money — a charge, a bill, a refund, a "
        "financial loss — and the other does not, the monetary issue is primary, since financial impact "
        "outweighs a non-monetary complaint when neither is flagged as more urgent; (3) otherwise, rank the two "
        "issues by this fixed category-importance order and let the higher-ranked one win: Security > Billing > "
        "Account Access > Bug Report > Integration/API > Legal/Compliance  > Feature Request > General Inquiry"
        "(this mirrors why Feature Request and General Inquiry are always Low priority on their own — they are "
        "the least urgent category of concern, so they only win the primary slot when paired with another "
        "Feature Request/General Inquiry issue, never against a Security, Billing, Account Access, Bug Report, "
        "Integration/API, or Legal/Compliance issue); (4) otherwise, if the two issues are the same rank (e.g. "
        "both Feature Request, or both General Inquiry), default to whichever concern the ticket raises FIRST as "
        "the primary issue. "
        "For example, for 'please add dark mode to the app, also I never received the account verification "
        "email', the feature request (dark mode) is mentioned first and involves no money, but Account Access "
        "outranks Feature Request in the category-importance order, so category must be Account Access, "
        "assigned_team Identity & Access, priority Medium (a single user's own access problem with an available "
        "workaround, per the Medium rubric) — NOT Feature Request/Product just because dark mode was mentioned "
        "first. The reasoning should read something like 'The user hasn't received their account verification "
        "email, which is the main issue since it blocks their access; the dark mode request they also mentioned "
        "will need to be looked at separately.' "
        "Contrast this with a ticket that adds explicit importance language to the dark mode request, e.g. "
        "'please add dark mode to the app, which is honestly the more important issue for me — also I never "
        "received the account verification email.' Here rule (1) fires because the ticket itself explicitly "
        "marks dark mode as the more important issue, so dark mode/Feature Request IS primary — category "
        "Feature Request, assigned_team Product, priority Low (feature requests are Low on their own) — even "
        "though Account Access would normally outrank Feature Request under rule (3). An explicit importance "
        "statement in the ticket (rule 1) always overrides the fixed category-importance order (rule 3); do not "
        "let the memorized pattern from the plain, no-stated-importance version of this example override an "
        "explicit importance statement when the ticket actually contains one. "
        "Classify by whichever issue wins under this test — the category, assigned_team, and priority fields must "
        "all correspond to whichever issue is primary; never let the reasoning name one issue as primary while "
        "the category/team fields reflect the other, losing issue. When the ticket raises a second issue that lost out, "
        "briefly mention it in the reasoning too, but in plain, everyday language — never say 'tiebreaker', a "
        "rule number/name, 'runner-up', or anything that sounds like routing logic; just say plainly that the "
        "other thing they mentioned will need to be looked at too/separately. For example, for 'I was charged "
        "twice but now I also can't log in', the reasoning field should say something like 'The user reports a "
        "duplicate charge, which is the main issue here since it involves money; the login problem they mentioned "
        "will need to be looked at separately.' — still one or two simple sentences a customer could read and "
        "understand, just naming both issues plainly instead of hiding the second one. "
        ...
        "This applies even when the bare word itself names a category, such as 'hacked', 'billing', "
        "'bug', or 'security' typed alone with nothing else — a single category-sounding word with no "
        "account, system, timeframe, or any other detail is still Case B, not an automatic Security/High (or "
        "other category) classification, because there is nothing concrete to route on: e.g. 'hacked' by "
        "itself gives no indication of which account, what happened, or when, so it is Unclassified/Human "
        "Triage/Low the same way 'broken' alone is, never Security/High just because the word itself sounds "
        "like a breach. "
        ...
        "If the ticket is understandable but genuinely ambiguous between two categories, pick the closer match and "
        "keep confidence below 0.6. Do not mention the other category considered, or any other internal decision "
        "mechanics, in the reasoning field — just give a simple, plain-language sentence about the ticket itself. "
        "The reasoning field must always be exactly one simple, clear sentence in plain language, written for a "
        "non-technical reader — never mention rule names, tiebreakers, category numbers, confidence thresholds, or "
        "any other internal routing mechanics. It should just say what the issue is and why it got this priority/"
        "team, in the same plain way you'd explain it to someone with no knowledge of the routing system. "
